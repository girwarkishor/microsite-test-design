...
var shadowIn = 'box-shadow: 5px 5px 10px #e5e5e5;transition: box-shadow 0.1s ease-in-out;';
var shadowOut = 'box-shadow: 0px 0px 0px #e5e5e5;transition: box-shadow 0.1s ease-in-out;';
...
function accordionToggle(param, parentOfParam) {
  if (param.classList[0] === 'card-accordion--radio') {
    ...
    var allAccordions = document.querySelectorAll('.card--accordion-content.opened');
    ...
    var allAccordions = document.querySelectorAll('.card--accordion-content.width100.opened');
...
function descriptionCardToggle(el) {
  ...
  if (el.classList.value === 'card-descriptive--link' && el.hasAttribute('data-description-toggle')) {
...
function secondaryNavControl(event) {
  ...
  if (event.target.classList[1] === 'scroller-left') {
    ...
    navPills.setAttribute('style', existingStyle + 'transform:translateX(' + shiftRight + '%);' + transtion)
    ...
    navPills.setAttribute('style', existingStyle + 'transform:translateX(-' + shiftLeft + '%);' + transtion)
...
function shareWidget(param) {
  ...
  if (!shareIcons[0].getAttribute('style') || shareIcons[0].getAttribute('style') === 'transform: translateY(0%); transition: 0.3s all ease-in-out;') {
    shareIcons.forEach(function (each) {
      each.setAttribute('style', 'transform: translateY(-' + position + '%); transition: 0.3s all ease-in-out;');
      position += 110;
    })
  } else if (shareIcons[0].getAttribute('style') === 'transform: translateY(-0%); transition: 0.3s all ease-in-out;') {
    shareIcons.forEach(function (each) {
      each.setAttribute('style', 'transform: translateY(' + 0 + '%); transition: 0.3s all ease-in-out;');
    })
  }
}
  ...
  if (param.classList.value === 'footer--accordion') {
    ...
    for (i = 4; i < totalFooterAccordions.length; i++) {
      if (i != (picked + 4)) {
        ...
        totalFooterAccordions[i - 4].children[0].classList.value = 'footer--accordion-indicator-only rotate-fa';
    ...
    for (j = 1; j < totalFooterAccordionsCount + 1; j++) {
      ...
      var array = []
      document.querySelectorAll('[data-toggle-id="' + j + '"]').forEach(function (each) {
        array.push(each)
      })
      ...
      if (param === array[0] || param.classList === array[1]) {
        ...
        document.querySelectorAll('[data-toggle-id="' + clicked + '"]')[1].style.display = 'block'
        document.querySelectorAll('[data-toggle-id="' + clicked + '"]')[0].children[0].classList.toggle('rotate-fa')
        document.querySelectorAll('[data-toggle-id="' + clicked + '"]')[1].classList.toggle('opened')
      }
...
function actOnClick(param) {
  ...
  var allRadioButtons = Array.from(document.querySelectorAll('input[type=radio]'));
  ...
  if (inputField && type) {
    if (type === false) {
      ...
      param.children[0].children[0].children[0].classList.toggle('checkbox-color')
      checkedUnchecked(inputField)
...
window.addEventListener('focusin', function (e) {
  if (e.target.hasAttribute('placeholder')) {
    if (e.target.getAttribute('class').includes('form--reset ')) {
      e.target.parentNode.children[0].setAttribute('class', 'form--placeholder fadeIn animated')
    } else if (e.target.getAttribute('class').includes('form--with-icon ')) {
      e.target.parentNode.parentNode.children[0].setAttribute('class', 'form--placeholder-with-icon fadeIn animated')
    }
  }
});
...
window.addEventListener('focusout', function (e) {
  if (e.target.hasAttribute('placeholder') && e.target.value === '') {
    if (e.target.getAttribute('class').includes('form--reset ')) {
      e.target.parentNode.children[0].setAttribute('class', 'form--placeholder fadeOut animated')
    } else if (e.target.getAttribute('class').includes('form--with-icon ')) {
      e.target.parentNode.parentNode.children[0].setAttribute('class', 'form--placeholder-with-icon fadeOut animated')
    }
  }
});
